# -*- coding: utf-8 -*-
"""
Tests for AskCommand multi-stock portfolio overlay integration.
"""
...
class TestAskCommandMultiStock(unittest.TestCase):
    ...
    def test_analyze_single_passes_requested_strategy_into_context(self):
        command = AskCommand()
        config = SimpleNamespace()
        message = self._message()
        captured = {}
        ...
        with patch("src.agent.factory.build_agent_executor", return_value=FakeExecutor()):
            with patch.object(command, "_resolve_strategy_name", return_value="chan_theory"):
